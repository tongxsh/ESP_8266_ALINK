...
extern os_timer_t smartConfigLed;
...
alink_err_t alink_connect_ap(void)
{
    ...
    wifi_set_opmode(STATION_MODE);
        ...
        ALINK_LOGI("*************alink_connect_ap*******start *************");
    if (esp_info_load(NVS_KEY_WIFI_CONFIG, &wifi_config, sizeof(struct station_config)) <= 0) {
        ...
        ALINK_LOGI("*************alink_connect_ap***end *****************");
 
    return ALINK_OK;
}

alink_err_t user_connect_ap(void)
{
    struct station_config wifi_config;
    wifi_set_opmode(STATION_MODE);
     
        ALINK_LOGI("*************user_connect_ap*******start *************");
    if (esp_info_load(NVS_KEY_WIFI_CONFIG, &wifi_config, sizeof(struct station_config)) <= 0) {
      ALINK_LOGI("*********************************");
        ALINK_LOGI("*    ENTER SAMARTCONFIG MODE    *");
        ALINK_LOGI("*********************************");
        ...
        if(aws_smart_config() == ALINK_OK){
             ALINK_LOGI("*       aws_smart_config OK       *");
        } else {
            ALINK_LOGI("*********************************");
            ALINK_LOGI("*       ENTER SOFTAP MODE       *");
            ALINK_LOGI("*********************************");
        ...
        ALINK_LOGI("*************user_connect_ap***end *****************");
